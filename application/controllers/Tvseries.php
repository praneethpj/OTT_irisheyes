...
class Tvseries extends Home_Core_Controller{
    ...
    public function home(){
        ...
        if($tv_series_publish =='1'):
            ...
            $config["total_rows_top_rated_series"] = $this->common_model->top_rated_series_tv_series_record_count();
            $config["per_page"] = 18;
            ...
            $data["all_published_videos"]   = $this->common_model->fetch_tv_series();
            $data["all_top_rated_series"]   = $this->common_model->fetch_top_rated_series();
            $data["links"]                  = $this->pagination->create_links();
            $data['total_rows']             = $config["total_rows"];
			$data["total_rows_top_rated_series"] = $config["total_rows_top_rated_series"];
            // seo
	...
	public function Allseries(){
		$data["all_published_series"]   = $this->common_model->fetch_Allseries();
		$data['canonical']          = base_url('allseries.html');
		$data['page_name']              = 'allseries';
		$this->load->view('theme/'.$this->active_theme.'/index',$data);
	}
}
