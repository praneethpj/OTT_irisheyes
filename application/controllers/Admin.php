...
class Admin extends Admin_Core_Controller {   
    ...
    function genre($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'add') {
            $data['name']           = $this->input->post('name');
            $data['vast_tag']       = $this->input->post('vast_tag');
            $data['description']    = $this->input->post('description');
            $data['slug']           = url_title($this->input->post('name'), 'dash', TRUE);
    ...
    // add videos or movies 
    function videos_home_add(){
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
        // start menu active/inactive section
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '66');
        // end menu active/inactive section
        $data['page_name']      = 'videos_home_add';
        $data['page_title']     = trans('videos_add'); 
        $this->load->view('admin/index', $data);
    }
    ...
    // edit videos or movies 
    function videos_home_edit($param1='',$param2=''){
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
            /* start menu active/inactive section*/
            $this->session->unset_userdata('active_menu');
            ...
            $data['param1']         = $param1;
            $data['param2']         = $param2;
            $data['page_name']      = 'videos_home_edit';
            $data['page_title']     = trans('video_edit').' | '.$this->common_model->get_title_by_videos_id($param1);
            $this->load->view('admin/index', $data);
    }
	...
	// add,edit videos or movies 
    function videos_home($param1 = '', $param2 = ''){
		
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
        // active menu session
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '88'); 
        ...
        if ($param1 == 'add') { 
            ...
            $data['meta_description']   = $this->input->post('meta_description');
            $data['tags']               = $this->input->post('tags');       
            $data['button1_name']               = $this->input->post('button1_name');  
            $data['button1_link']               = $this->input->post('button1_link');  
            $data['button2_name']               = $this->input->post('button2_name');  
            $data['button2_link']               = $this->input->post('button2_link');  
                    
            $this->db->insert('videos_home', $data);
            $insert_id = $this->db->insert_id();
             //echo "<pre>";print_r($insert_id);exit;   
            $slug                       = url_title($this->input->post('slug'), 'dash', TRUE);
            $slug_num                   = $this->common_model->slug_num('videos_home',$slug);
            if($slug_num > 0){
                $slug= $slug.'-'.$insert_id;
            }
            $data_update['slug']        = $slug;            
            if(isset($_FILES['thumbnail']) && $_FILES['thumbnail']['name']!=''){
                move_uploaded_file($_FILES['thumbnail']['tmp_name'], 'uploads_home/video_thumb/'.$insert_id.'.jpg');                
            }

            if(isset($_FILES['poster_file']) && $_FILES['poster_file']['name']!=''){
                move_uploaded_file($_FILES['poster_file']['tmp_name'], 'uploads_home/poster_image/'.$insert_id.'.jpg');                                
            }

            if($this->input->post('thumb_link')!=''){ 
                $image_source           =   $this->input->post('thumb_link');
                $save_to                =   'uploads_home/video_thumb/'.$insert_id.'.jpg';           
                $this->common_model->grab_image($image_source,$save_to);
            }

            if($this->input->post('poster_link')!=''){ 
                $image_source       =   $this->input->post('poster_link');
                $save_to            =   'uploads_home/poster_image/'.$insert_id.'.jpg';           
                $this->common_model->grab_image($image_source,$save_to);
            }

            $this->db->where('videos_id', $insert_id);
            $this->db->update('videos_home', $data_update);
            ...
            $this->session->set_flashdata('success', trans('add_success'));
            redirect(base_url() . 'admin/file_and_download_home/'.$insert_id, 'refresh');
        }
        else if ($param1 == 'update') {
            $data['imdbid']                 = $this->input->post('imdbid');          
            $data['title']                  = $this->input->post('title');
            ...
            $data['tags']               = $this->input->post('tags');
            $this->db->where('videos_id', $param2);
            $this->db->update('videos_home', $data);
            $this->db->where('videos_id', $param2);
            ...
            $data_update['slug']               = $slug;            
            if(isset($_FILES['thumbnail']) && $_FILES['thumbnail']['name']!=''){
                move_uploaded_file($_FILES['thumbnail']['tmp_name'], 'uploads_home/video_thumb/'.$param2.'.jpg');                
            }

            if(isset($_FILES['poster_file']) && $_FILES['poster_file']['name']!=''){
                move_uploaded_file($_FILES['poster_file']['tmp_name'], 'uploads_home/poster_image/'.$param2.'.jpg');                                
            }

            if($this->input->post('thumb_link')!=''){ 
                $image_source =$this->input->post('thumb_link');
                $save_to = 'uploads_home/video_thumb/'.$param2.'.jpg';           
                $this->common_model->grab_image($image_source,$save_to);
            }

            if($this->input->post('poster_link')!=''){ 
                $image_source =$this->input->post('poster_link');
                $save_to = 'uploads_home/poster_image/'.$param2.'.jpg';           
                $this->common_model->grab_image($image_source,$save_to);
            }
            ...
            $this->session->set_flashdata('success', trans('update_success'));
            redirect(base_url() . 'admin/videos_home_edit/'.$param2, 'refresh');
        }
        // filter
        $title          = $this->input->get('title');
        $release        = $this->input->get('release');
        ...
        $total_rows = $this->common_model->get_videos_num_rows_home($filter);
        // page
        $config                     = $this->common_model->pagination();
        $config["base_url"]         = base_url() . "admin/videos_home?".$search_string;
        $config["total_rows"]       = $total_rows;
        $config["per_page"]         = 10;
        $config["uri_segment"]      = 3;          
        //$config['use_page_numbers'] = TRUE;
        ...
        $data['last_row_num']       = $this->uri->segment(3);
        $page                       = $this->input->get('per_page');//($this->uri->segment(3)) ? $this->uri->segment(3) : 0;   
        $data["videos"]             = $this->common_model->get_videos_home($filter,$config["per_page"], $page);
        $data["links"]              = $this->pagination->create_links();
        $data['total_rows']         = $config["total_rows"];
        $data['page_name']          = 'videos_manage_home';
        $data['page_title']         = trans('videos_manage');             
        $this->load->view('admin/index', $data);
    }
    ...
    function file_and_download_home($param1 = '', $param2 = ''){
    if ($this->session->userdata('admin_is_login') != 1)
        redirect(base_url(), 'refresh');
        /* start menu active/inactive section*/
        $this->session->unset_userdata('active_menu');
        ...
        if ($param1 == 'update') {
            ...
            for($i=0;$i<sizeof($video_file_type);$i++){                
                ...
                if($video_file_type[$i]     == 'upload'){
                   $file_data['source_type'] = $this->common_model->get_extension($video_file[$i]);
                   copy('uploads_home/temp/'.$video_file[$i], 'uploads_home/videos/'.$video_file[$i]);
                }
                $file_data['file_url'] = $video_file[$i];
                $this->db->insert('video_file', $file_data);
                //var_dump($file_data);
        ...
        if ($param1 == 'edit'):
            ...
            $data['param1']         = $param1;
            $data['param2']         = $param2;
            $data['slug']           = $this->common_model->get_slug_by_videos_id_home($param1);
            $data['page_name']      = 'file_and_download_home';
            $data['page_title']     = trans('file_and_download').' | '.$this->common_model->get_title_by_videos_id_home($param1);
            $this->load->view('admin/index', $data);
        endif;
    }
    ...
    function live_tv_category($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'add') {            
            ...
            $data['live_tv_category_vast_tag']             = $this->input->post('live_tv_category_vasttag');    
    ...
    public function movie_upload_home(){
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if($source == 'upload'):
            if($this->form_validation->run() == FALSE):
                ...
                //upload configuration
                $NewFileName                = $videos_id.'-'.uniqid(); //new file name
                $config['upload_path']      = 'uploads_home/videos/';
                $config['allowed_types']    = 'mp4|webm|mkv|m3u8';
                $config['file_name']        = $NewFileName;
                $config['max_size']         = 0;
                //$this->load->library('upload', $config);
                ...
                if($this->upload->do_upload('videofile')):
                    ...
                    $data['file_source']    = $file_ext;
                    $data['source_type']    = 'upload';                    
                    $data['file_url']       = base_url().'uploads_home/videos/'.$file_name;
                    
                    $this->db->insert('video_file_home', $data);
                    $insert_id              = $this->db->insert_id();
                    $insert_status          = TRUE;
                    $this->session->set_flashdata('success', trans('video_add_success'));
                else:
	...
	function home_page_video() {
		$data['page_info']  = 'Home Page Video';
		$data['page_name']  = 'home_page_video';
		$data['page_title'] = trans('home_page_video');
		$this->load->view('admin/index', $data);
	}
