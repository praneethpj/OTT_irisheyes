...
//dJAX - Paypal lib start
require(APPPATH.'third_party/braintree-php-6.3.0/vendor/braintree/braintree_php/lib/Braintree.php');
//dJAX - Paypal lib end
...
class V100 extends RestController{ 
   ...
   // login function
    public function login_post() {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL) && $password !='' && $password !=NULL):            
            ...
            if ($login_status):
                ...
                // $response['phone']      = $user_info->phone; //As per client confirmation on 11-08-2021 call we commented this.
    ...
    public function update_profile_post() {
        ...
        // $this->form_validation->set_rules('phone', 'Phone', 'trim'); //As per client confirmation on 11-08-2021 call we commented this.
        if ($this->form_validation->run() == FALSE):
            ...
            // $phone                      =   $this->input->post('phone'); //As per client confirmation on 11-08-2021 call we commented this.      
            ...
            if ($is_valid_user_id):
                ...
                /*if(!empty($phone) && $phone !='' && $phone !=NULL): //As per client confirmation on 11-08-2021 call we commented this.
                    $data['phone']           =   $phone;
                endif;*/
    ...
    public function search_get() {
        ...
        $tv_category_id     =   $this->input->get('tv_category_id');
    ...
    public function password_reset_post() {        
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)):         
            ...
            if($user_exist):
                ...
                $this->load->model('email_model');
                if($this->email_model->android_password_reset_email($email,$token)):
   ...
   //dJAX - Comment it start
    ...
    public function firebase_auth_post() {        
        ...
        if($uid !='' && $uid !=NULL):         
            ...
            // print_r($user_exist_by_email);die("sd");    
            ...
            /*elseif($user_exist_by_phone):
                ...
                // $firebase_data['is_password_set']    = 1;
    ...
    //dJAX - Request function end

    //dJAX - Report issue start
    ...
    //dJAX - Report issue end

    //dJAX - SDK Brain tree paypal client token generation process start
    public function client_token_get() {
        ...
        {
            $response['status']     = 'Error';
            $response['message']    = 'Issue in client token generating...';    
            $this->response($response,200);
        }

    }
    //dJAX - SDK Brain tree paypal client token generation process end
    //dJAX - SDK Brain tree paypal transaction process start
    public function noncetoken_get() {
        $nonceFromTheClient       =   $this->input->get('payment_method_token');
        $payment_amount           =   $this->input->get('amount');
        $device_data              =   $this->input->get('device_data');
        $device_data = json_decode($device_data);
        $device_data = $device_data->correlation_id;
        $gateway = new Braintree\Gateway([
            'environment'=> $this->config->item('environment'),
            'merchantId' => $this->config->item('merchantId'),
            'publicKey'  => $this->config->item('publicKey'),
    ...
    //dJAX - SDK Brain tree paypal transaction process end

    //dJAX - Store Payment Info to database table start
    public function store_payment_info_post() {

        $payment_info = json_encode($this->input->post());
        $data['plan_id']                    =   $this->input->post('plan_id');
        $data['user_id']                    =   $this->input->post('user_id');
        $data['price_amount']               =   $this->input->post('paid_amount');
        ...
        if (!empty($insert_id)) {
            ...
            $response['status']         = 'Error';
            $response['message']        = 'Error in inserting the data';            
            $this->response($response,200);}
        }
    //dJAX - Store Payment Info to database table end
