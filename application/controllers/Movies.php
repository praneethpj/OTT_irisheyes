...
class Movies extends Home_Core_Controller{
    public function __construct(){
        parent::__construct();
    }
    
    public function index($slug=''){
        $movies_publish                =   $this->db->get_where('config' , array('title'=>'movies_publish'))->row()->value;        
    	if($movies_publish =='1' && $slug !='' && $slug !=NULL):
    ...
    public function home(){
		...
		$config = array();
		$config["base_url"] = base_url() . "movies/home";
		$config["total_rows"] = $this->common_model->movies_record_count();
		$config["total_rows_top_rated_movies"] = $this->common_model->top_rated_series_movies_record_count();
		$config["per_page"] = 18;
		...
		$data["all_published_videos"]   = $this->common_model->fetch_movies();
		$data["all_top_rated_movies"]   = $this->common_model->fetch_top_rated_movies();
		$data["links"]                  = $this->pagination->create_links();
		$data['total_rows']             = $config["total_rows"];
		$data["total_rows_top_rated_movies"] = $config["total_rows_top_rated_movies"];
    ...
    public function watch($slug='',$param1='',$param2=''){
        ...
        if($movies_publish =='1' && $slug !='' && $slug !=NULL && $video_is_published):
	...
	public function Allmovies(){
		$data["all_published_movies"]   = $this->common_model->fetch_Allmovies();
		$data['canonical']          = base_url('allmovies.html');
		$data['page_name']              = 'allmovies';
		$this->load->view('theme/'.$this->active_theme.'/index',$data);
	}
}
