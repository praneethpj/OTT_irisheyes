 ...
 * Ayozat
 *
 * Ayozat-Movie & TV-Series and Live TV platform
 *
 * @package     Ayozat Limited
 * @author      Umesh Perera, Lazar Andjelkovic , Alex Perera
 * @copyright   Copyright (c) Ayozat Limited,
 * @license     
 * @link        http://www.ayozat.com
 * @link        support@ayozat.co.uk
 *
 **/
...
class Common_model extends CI_Model {
    ...
    function get_category_name($category_id=''){
		...
		function get_Star_img($id = '')
		{
			if(file_exists('uploads/star_image/'.$id.'.jpg'))
				$image_url  =   base_url().'uploads/star_image/'.$id.'.jpg';
			else
	...
	public function all_publishe_tvseries($limit=12,$page='')
    {        
	...
	public function carousel_live_tv_videos($limit=5,$page='')
    {
	...
	function get_video_thumb_url_live($poster = '')
    {
	...
	public function get_videos_movie_tv_series($array=array(),$limit=NULL, $start=NULL)
    {
		if($array !='' && $array !=NULL && !empty($array))
			if($array == 'ayozat'){
				...
				$this->db->limit($limit,$start);
				$this->db->group_start();
				$this->db->like('title',$array);
				$this->db->or_like('seo_title',$array);
				$this->db->or_like('focus_keyword',$array);
				$this->db->or_like('meta_description',$array);
				$this->db->or_like('tags',$array);
				$this->db->group_end();
				$query = $this->db->get('videos');
			}
	...
	public function get_videos_live($array=array(),$limit=NULL, $start=NULL)
    {
		if($array !='' && $array !=NULL && !empty($array)){}
			if($array == 'ayozat'){
				$this->db->where('publish', '1');
				...
				$this->db->limit($limit,$start);
				$this->db->group_start();
				$this->db->like('tv_name',$array);
				$this->db->or_like('seo_title',$array);
				$this->db->or_like('slug',$array);
				$this->db->or_like('focus_keyword',$array);
				$this->db->or_like('meta_description',$array);
				$this->db->or_like('tags',$array);
				$this->db->group_end();
	...
	public function get_videos_num_rows_movies($array=array())
    {
        if($array !='' && $array !=NULL && !empty($array))
			if($array == 'ayozat'){
				$this->db->where('publish', '1');
	...
	public function temp_get_watch_again_first_row($limit=2)
    {        
        $this->db->where('is_tvseries','0');
        $this->db->where('publication', '1');
	...
	public function temp_get_watch_series_again_first_row($limit=4)
    {        
        $this->db->where('is_tvseries','1');
        $this->db->where('publication', '1');
        $this->db->order_by("last_ep_added","desc");
	...
	public function temp_get_watch_footer_first_row($limit=1)
    {        
        $this->db->where('is_tvseries','0');
        $this->db->where('publication', '1');
        $this->db->order_by("last_ep_added","desc");
	...
	public function get_videos_num_rows_videos($array=array())
    {
        if($array !='' && $array !=NULL && !empty($array))
			if($array == 'ayozat'){
				$this->db->where('publication','1');
	...
	public function temp_get_watch_again($limit=2)
    {        
        $this->db->where('is_tvseries','0');
        $this->db->where('publication', '1');
	...
	public function temp_get_watch_footer_again($limit=1)
    {        
        $this->db->where('is_tvseries','0');
        $this->db->where('publication', '1');
        $this->db->order_by("today_view","desc");
	...
	public function get_top_rated_series($limit=12)
    {        
        $this->db->where('is_tvseries','1');
        $this->db->where('publication', '1');
        $this->db->order_by("total_rating","desc");
	...
	public function get_tv_genres($limit=5)
    {  
	...
	public function get_top_rated_movies($limit=12)
    {  
        $this->db->where('publication', '1');
        $this->db->where('is_tvseries', '0');
        $this->db->order_by("total_rating","desc");
	...
	public function get_max_rated_movies($limit=18)
    {  
        $this->db->where('publication', '1');
        $this->db->where('is_tvseries', '0');
        $this->db->order_by("total_rating","desc");
	...
	public function latest_live_tv($limit=12,$page='')
    	{
        $this->db->where('publish', '1');
        $this->db->order_by("live_tv_category_id","desc");
        $this->db->limit($limit);
    ...
    public function latest_published_tv_series($limit=12,$page='')
    {
        $this->db->where('publication', '1');
        $this->db->where('is_tvseries', '1');
        $this->db->order_by("last_ep_added","DESC");
	...
	//homepage video
	public function get_videos_num_rows_home($array=array())
    {
        if($array !='' && $array !=NULL && !empty($array))
            $this->db->like($array);
        $query = $this->db->get('videos_home');
        return $query->num_rows();
    }

	public function get_videos_home($array=array(),$limit=NULL, $start=NULL)
    {
        if($array !='' && $array !=NULL && !empty($array))
            $this->db->like($array);
        $this->db->order_by("videos_id","desc");
	...
	function get_video_thumb_url_home($videos_id = '')
    {
            if(file_exists('uploads_home/poster_image/'.$videos_id.'.jpg'))
        $image_url  =   base_url().'uploads_home/poster_image/'.$videos_id.'.jpg';
        else
            $image_url  =   base_url().'uploads/default_image/thumbnail.jpg';
	...
	function get_video_thumb_url_admin($videos_id = '')
    {
	...
	function get_slug_by_videos_id_home($videos_id=''){
        $video =$this->db->get_where('videos_home' , array('videos_id'=>$videos_id));
        if($video->num_rows() >0){
	...
	function get_title_by_videos_id_home($videos_id=''){
        $video =$this->db->get_where('videos_home' , array('videos_id'=>$videos_id));
        if($video->num_rows() >0){
	...
	function get_star_desc_by_id($star_id)
    {
        if(is_numeric($star_id) && $star_id !=''):
            $query  =   $this->db->get_where('star' , array('star_id' => $star_id));
            $res    =   $query->result_array();
            foreach($res as $row)           
                return $row['star_desc'];
        else:
            return $star_id;
        endif;
    }

	function get_star_type_by_id($star_id)
    {
        if(is_numeric($star_id) && $star_id !=''):
            $query  =   $this->db->get_where('star' , array('star_id' => $star_id));
            $res    =   $query->result_array();
	...
	public function get_first_video_details_videos_id_home($videos_id=''){
        $this->db->order_by('order', $this->video_file_order());
        $this->db->limit(1);
        return $this->db->get_where('video_file_home', array('videos_id'=>$videos_id))->row();
    }

    
    public function movies_record_count()
    {
		return $this->db->get_where('videos', array('is_tvseries'=>'0'))->num_rows();
    }

	public function top_rated_series_movies_record_count()
    {
        $this->db->where('is_tvseries','0');
        $this->db->where('publication', '1');
        $this->db->order_by("total_rating","desc");
	...
	public function top_rated_series_tv_series_record_count()
    {
		$this->db->where('is_tvseries','1');
        $this->db->where('publication', '1');
        $this->db->order_by("videos_id","desc");
	...
	public function top_rated_movies_record_count()
    {
		$this->db->where('is_tvseries','0');
        $this->db->where('publication', '1');
        $this->db->order_by("videos_id","desc");
   ...
   public function fetch_tv_series() {
		$this->db->where('is_tvseries','1');
        $this->db->where('publication', '1');
        $this->db->order_by("videos_id","desc");
        // $this->db->limit($limit,$start);
   ...
   public function fetch_movies() {
	...
	public function fetch_Allseries() {
		$this->db->where('is_tvseries','1');
		$this->db->where('publication', '1');
		$this->db->order_by("last_ep_added","desc");
		$query = $this->db->get("videos");
   ...
   public function fetch_top_rated_series() {
	$this->db->where('is_tvseries','1');
	$this->db->where('publication', '1');
	$this->db->order_by("total_rating","desc");
	// $this->db->limit($limit);
...
//    report
	...
	public function get_header_image_movies($limit=1){
        $response   =   array();
        $this->db->where('is_tvseries =','0');
		$this->db->where('publication','1');
        $this->db->order_by("videos_id","desc");
        $this->db->limit(1);
        $response_movies     = $this->db->get('videos')->result_array();
        return $response_movies;
    }

	public function get_header_image_tv_series($limit=1){
        $response   =   array();
        $this->db->where('is_tvseries =','1');
		$this->db->where('publication','1');
        $this->db->order_by("videos_id","desc");
        $this->db->limit(1);
        $response_tv_series    = $this->db->get('videos')->result_array();
        return $response_tv_series;
    }

	function get_header_image_thumb_url($videos_id = '')
    {
        if(file_exists('uploads/video_thumb/'.$videos_id.'.jpg'))
             $image_url  =   base_url().'uploads/video_thumb/'.$videos_id.'.jpg';
        else
