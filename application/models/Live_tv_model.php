...
class Live_tv_model extends CI_Model {
	...
	public function get_browsed_live_tv() {
		return $this->db->get('live_tv')->result_array();
	}

    public function get_featured_live_tv() {
    ...
    public function get_all_live_tv() {
     ...
     public function get_all_live_tv_temp($limit=25) {
        $this->db->where('publish','1');
        $this->db->limit($limit);
        return $this->db->get('live_tv')->result_array();
    }

 	public function get_all_live_tv_tempp() {
        $this->db->where('publish','1');
        //$this->db->limit($limit);
        return $this->db->get('live_tv')->result_array();
    }
    ...
    public function get_live_tv_category_vast_tag_by_category_id($live_tv_category_id='') {
        $result = "not found";
        $this->db->where('live_tv_category_id',$live_tv_category_id);
        $query = $this->db->get('live_tv_category');
        if($query->num_rows() > 0)
            $result = $query->row()->live_tv_category_vast_tag;
        return $result;
    }
}
